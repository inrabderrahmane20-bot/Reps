...
export function FirstVisitLanguageModal() {
  ...
  const dialog = (
    <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-ink-900/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={t('title')}>
      <div
        className="w-full max-w-md rounded-[28px] border border-white/60 bg-sand-50 p-6 shadow-float sm:p-8 arch-top"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-3">
          <span className="grid h-14 w-14 place-items-center rounded-arch bg-majorelle-600 text-white">
        ...
        <div role="radiogroup" aria-label={t('title')} className="mt-6 space-y-2.5">
          {locales.map((l) => {
        ...
        <button
          ...
          disabled={!selected}
          className="mt-6 w-full rounded-full bg-majorelle-600 py-3 text-[15px] font-semibold text-white transition-colors hover:bg-majorelle-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {t('continue')}
        </button>
      </div>
