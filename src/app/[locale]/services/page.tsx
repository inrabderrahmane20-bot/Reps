...
export default function ServicesPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-8">
      <h1 className="font-display text-3xl font-semibold text-ink-900">Find trusted local services</h1>
      <p className="mt-1.5 max-w-xl text-sm text-ink-500">Browse providers recommended by the Marrakech community.</p>
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <label className="flex flex-1 items-center gap-2 rounded-full bg-white px-4 py-3 shadow-card"><Search size={18} className="text-ink-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search services" className="w-full bg-transparent text-sm outline-none" /></label>
        <button type="button" onClick={() => setFiltersOpen(!filtersOpen)} className="inline-flex items-center justify-center gap-2 rounded-full border border-ink-900/10 bg-white px-5 py-3 text-sm font-semibold text-ink-700 hover:bg-sand-100"><SlidersHorizontal size={16} />Filters</button>
      </div>
